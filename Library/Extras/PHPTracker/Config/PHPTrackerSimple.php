 ...
 * @subpackage Config
 */
class PHPTrackerConfigSimple implements PHPTrackerConfigInterface
{
    /**
     ...
     * @param boolean $needed If the config key is missing and $needed is true, this MUST throw PHPTrackerConfigMissingException.
     * @param mixed $default If the config key is missing and $needed is false, this MUST return $default.
     * @throws PHPTrackerConfigMissingException When the config value is required and missing.
    ...
    public function get( $config_name, $needed = true, $default = null ){
        if(!isset($this->config_values[$config_name])){
            if($needed){
                throw new PHPTrackerConfigMissingException( "Value not found: $config_name" );
            }
            return $default;
        }
     ...
     * @param boolean $needed If a config key is missing and $needed is true, this MUST throw PHPTrackerConfigMissingException.
     * @param mixed $default If a config key is missing and $needed is false, this MUST return the according $default.
     * @throws PHPTrackerConfigMissingException When a config value is required and missing.
