...
class PHPTrackerCore{
     ...
     * @throws PHPTrackerException When the announce-list is empty.
     * @return string
     */
    public function createTorrent($file_path, $size_piece=262144, $basename=null){
        $torrent=new PHPTrackerTorrent( new PHPTrackerFile($file_path), $size_piece, $file_path, $basename );
        ...
        if(empty($announce)){
            throw new PHPTrackerException('Empty announce list!');
        }
     ...
     * (not in the GET!). This is a number representing seconds for the client to
     ...
     *
     * @param PHPTrackerConfigInterface $get Config-like representation of the CGI parameters (aka. GET) sent.
     * @return string
     */
    public function announce(PHPTrackerConfigInterface $get){
        try{
            try{
            ...
            }catch(PHPTrackerConfigMissingException $e){
            ...
            return PHPTrackerBencodeBuilder::build($announce_response);
        }catch(Exception $e){
    ...
    protected function announceFailure($message){
        return PHPTrackerBencodeBuilder::build(array(
