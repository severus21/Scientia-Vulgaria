...
class PHPTrackerTorrent{
    ...
    /**
     * Concatenated ahshes of each piece of this file.
     *
     * @var string
     */
    protected $pieces;
     ...
     *
     * @param PHPTrackerFile $file To intialize 'file' attribute.
     ...
     */
    public function __construct( PHPTrackerFile $file, $size_piece, $file_path=null, $name=null, $length=null, $pieces=null, $info_hash=null){
        if(0>=$size_piece=intval($size_piece)){
            throw new PHPTrackerException('Invalid piece size: ' . $size_piece);
        }
    ...
    public function __get($attribute){
        switch($attribute){
            ...
            case 'size_piece':
                ...
                break;
            default:
                throw new PHPTrackerException( "Can't access attribute $attribute of " . __CLASS__ );
        }
    }
    ...
    protected function calculateInfoHash(){
        ...
        return sha1( PHPTrackerBencodeBuilder::build( array(
    ...
    public function createTorrentFile(array $announce_list){
        ...
        return PHPTrackerBencodeBuilder::build($torrent_data);
    }
    ...
    public function readBlock($piece_index, $block_begin, $length){
        if($piece_index>ceil($this->__get('length')/$this->size_piece)-1){
            throw new PHPTrackerException( 'Invalid piece index: ' . $piece_index );
        }
