...
class PHPTrackerSeederClient{
     ...
     *
     * @throws PHPTrackerSeederSocketException If the accepting is unsuccessful.
    ...
    public function socketAccept($listening_scket){
        if(false===($this->communication_socket=socket_accept($listening_scket))){
            ...
            throw new PHPTrackerSeederSocketException( 'Socket accept failed: ' . socket_strerror( socket_last_error( $this->listening_socket ) ) );
        }
     ...
     * @throws PHPTrackerSeederSocketException If reading fails.
     * @throws PHPTrackerSeederCloseConnectionException If client closes the connection.
     * @return string
     */
    public function socketRead( $wanted_length ){
        ...
        while(strlen($message)<$wanted_length){
            if(false===($buffer=socket_read($this->communication_socket, min( $wanted_length-strlen($message), 2048 ), PHP_BINARY_READ))){
                throw new PHPTrackerSeederSocketException('Socket reading failed: '.socket_strerror($err_no=socket_last_error($this->communication_socket))." ($err_no)");
            }
            if(''==$buffer){
                throw new PHPTrackerSeederCloseConnectionException('Client closed the connection.');
            }
            $message.=$buffer;
        }
